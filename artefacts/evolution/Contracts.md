**Taxonomy.md** is the source of truth for what the system models. Every design decision in `Math.md` traces back to a structural fact in the taxonomy. When in doubt about meaning, the taxonomy wins.
...
2. **No additions.** If a feature is not in `Math.md`, it does not get built, even if `Taxonomy.md` describes it. The scope boundaries in Section 9 of `Math.md` are explicit. Deferred means deferred.

3. **No simplifications.** If `Math.md` specifies a computation, it is performed exactly as written. No approximations, no shortcuts, no "this is equivalent." The frozen constants are frozen. The numerical protocol is frozen. The PRNG seed is frozen.
